// interactions-full-v14.1.js — Crowd reactions + joiners + header typing + auto replies
(function () {
  'use strict';
  ...
  function renderReactions(bubbleEntry, reactions) {
    ...
    bubbleEntry.el
      .querySelector('.tg-bubble-content')
      ?.appendChild(pill);
  }
  ...
  function autoReactToMessage(message) {
    ...
    if (Math.random() < 0.4 && window.identity) {
      ...
      for (let i = 0; i < crowdClicks; i++) {
        ...
        const r =
          message.reactions[
            Math.floor(Math.random() * message.reactions.length)
          ];
    ...
    if (!window.TGRenderer.MESSAGE_MAP) return;

    const bubbleEntry =
      window.TGRenderer.MESSAGE_MAP.get(message.id);
  ...
  function simulateJoinerReply(joinerPersona) {
    ...
    const randomComment =
      window.realismEngineV12Pool[
        Math.floor(Math.random() *
        window.realismEngineV12Pool.length)
      ];
  ...
  function autoSimulate() {
    ...
    if (
      !window.realismEngineV12Pool ||
      window.realismEngineV12Pool.length === 0
    ) return;
  ...
  console.log(
    "✅ Interactions full v14.1 — crowd reactions + joiner replies + header typing fully integrated."
  );
