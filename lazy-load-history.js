// full-hybrid-loader-v23.js — Full history + realism engine + identity personas + live + joiners
(function(){
"use strict";
...
// Pick random reply target for threaded replies
...
async function loadBatch(batch, pool){
    const typers = [];
    for(let i=0;i<Math.min(3,batch.length);i++) typers.push(batch[i].persona);
    if(typers.length) await showTypingHeader(typers);
...
async function prepareFullHistory(totalCount=20000){
    ...
    while(window.realism.POOL.length<totalCount){
        ...
        if(!comment.persona) comment.persona = window.identity.getRandomPersona();
...
async function setupFullHybridLoader(){
    ...
    const history = await prepareFullHistory();

    // Initial render: all historical messages
    let index = 0;
    while(index < history.length){
        const batch = history.slice(index,index+SCROLL_BATCH_SIZE);
        await loadBatch(batch, history);
        index += SCROLL_BATCH_SIZE;
    }
    ...
    container.addEventListener("scroll", async()=>{
        ...
        if(distance < 300 && index < history.length){
            loading = true;
            const batch = history.slice(index,index+SCROLL_BATCH_SIZE);
            await loadBatch(batch, history);
            index += SCROLL_BATCH_SIZE;
