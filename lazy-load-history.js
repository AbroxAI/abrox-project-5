// realism-history-live-fixed-v12.js — Fast preloaded historical chat
(function(){
"use strict";
...
const BATCH_SIZE = 200; // render messages in batches
...
function timestamp(day){
    ...
    if(t.getTime() <= lastTime) t = new Date(lastTime + rand(20000,90000));
    lastTime = t.getTime();
    return t;
}
...
function simulateInlineReactions(messageId,count=1){
    ...
    for(let i=0;i<count;i++){
        ...
        window.TGRenderer.appendReaction(messageId,{emoji:reaction,count:1});
    }
}

/* BATCHED HISTORICAL PRELOAD */
function preloadHistory(){
    ...
    function renderBatch(){
        const batch = timeline.slice(index, index + BATCH_SIZE);
        batch.forEach(item=>{
            if(item.type==="join"){
                ...
                window.TGRenderer.prependMessage(
                    item.persona,
                    "",
                    {id, timestamp:item.timestamp, type:"system", event:"join", sticker:true, reactionPill:maybe(.4)}
                );
                return;
            }
            ...
            window.TGRenderer.prependMessage(
                ...
                text,
                {
                    ...
                    type:"historic",
                    bubblePreview:true,
                    replyPreview: parentId?true:maybe(.35),
                    parentId,
                    avatar:persona.avatar,
                    name:persona.name,
                    reactionPill:maybe(.3)
                }
            );
            messageIds.push(id);

            if(maybe(.35)) simulateInlineReactions(id,rand(1,3));
        });
        ...
        if(index < timeline.length){
            ...
            console.log("✅ Historical chat fully loaded (batched)");
        }
    }
...
async function postMessage(item){
    ...
    window.TGRenderer.appendMessage(
        ...
        item.text,
        {
            ...
            avatar:persona.avatar,
            name:persona.name,
            reactionPill:maybe(.25)
        }
    );
    if(maybe(.4)) simulateInlineReactions(msgId,rand(1,3));
}
...
function ensurePool(min=10000){
    ...
    while(window.realism.POOL.length<min){
        window.realism.POOL.push({text:generateComment(), timestamp:new Date()});
...
/* INIT */
async function init(){
    while(!window.identity?.getRandomPersona || !window.TGRenderer?.prependMessage || !window.queuedTyping){
        await new Promise(r=>setTimeout(r,50));
    }
    if(!window.realism) window.realism={};
    ensurePool(10000);
    preloadHistory(); // batched, fast
    simulateCrowdBurst(120);
    console.log("✅ Realism engine v12 ready (batched historical messages)");
}
