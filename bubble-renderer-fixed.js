// bubble-renderer-telegram2026.js — Full Telegram 2026 Style
(function () {
"use strict";
...
function init() {
  ...
  function appendJoinSticker(newMembers=[]){
    ...
    else if(newMembers.length===2) text = `${newMembers[0].name} & ${newMembers[1].name} joined the group`;
    else {
      const extra = newMembers.length - 2;
      text = `${newMembers[0].name}, ${newMembers[1].name} & ${extra} others joined the group`;
    }
  ...
  /* ================= PUBLIC API ================= */
  window.TGRenderer = { appendMessage, appendJoinSticker, lockInput, unlockInput, getPinnedMessageId:()=>PINNED_MESSAGE_ID, createReactionPill };

  console.log('✅ bubble-renderer: Telegram 2026 style');
