// identity-personas-v7-fixed-full.js — ULTRA-REALISTIC UNIQUE AVATARS + HEADER-READY
(function(){
"use strict";
...
function initializeAvatarPool(){
  ...
  for(let i=MIXED_AVATAR_POOL.length-1;i>0;i--){ const j=Math.floor(Math.random()*(i+1)); [MIXED_AVATAR_POOL[i],MIXED_AVATAR_POOL[j]]=[MIXED_AVATAR_POOL[j],MIXED_AVATAR_POOL[i]]; }
}
...
(function(){ try{ const raw=localStorage.getItem(AVATAR_PERSIST_KEY); if(raw){ const arr=JSON.parse(raw); if(Array.isArray(arr)) arr.forEach(u=>UsedAvatarURLs.add(u)); } }catch(e){} })();
function saveUsedAvs(){ try{ localStorage.setItem(AVATAR_PERSIST_KEY,JSON.stringify(Array.from(UsedAvatarURLs))); }catch(e){} }
...
(function fillRemaining(){ 
  const batch=500; 
  function batchFill(){ 
    const toDo=Math.min(batch,SyntheticPool.length<TOTAL_PERSONAS?TOTAL_PERSONAS-SyntheticPool.length:0); 
    ...
    else if(readyResolve) readyResolve(true); 
  } 
