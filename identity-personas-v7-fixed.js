...
if(raw){ const arr=JSON.parse(raw); if(Array.isArray(arr)) arr.forEach(u=>UsedAvatarURLs.add(u)); }
}catch(e){ }
})();
function saveUsedAvs(){ try{ localStorage.setItem(AVATAR_PERSIST_KEY,JSON.stringify(Array.from(UsedAvatarURLs))); }catch(e){} }
setInterval(saveUsedAvs,100060*2);
...
if(maybe(0.6)) base+=" "+rand(999);
if(maybe(0.45)) base=base.replace(/\s+/g,maybe(0.5)?"":".");
if(maybe(0.5)) base+=" "+random(EMOJIS);
}
let candidate=base.trim(), guard=0;
while(UsedNames.has(candidate)&&guard<10){ candidate=base.trim()+""+rand(9999); guard++; }
...
INITIAL POOL
===================================================== /
const INITIAL_SYNC=Math.min(800, Math.floor(TOTAL_PERSONAS0.3));
let readyResolve;
