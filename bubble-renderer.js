// bubble-renderer-fixed-v2.js — FINAL FIXED + REACTIONS + TREND + JOIN STICKER + INPUT LOCK + NEW MESSAGE PILL + AUTO-SCROLL
(function () {
  'use strict';
  ...
  function init() {
    ...
    const inputBar = document.getElementById('tg-input-bar') || document.querySelector('.tg-input-bar');
    ...
    function createBubble(persona, text, opts = {}) {
      ...
      const trending = opts.trending || false;

      insertDateSticker(timestamp);

      const wrapper = document.createElement('div');
      wrapper.className = `tg-bubble ${type}` + (trending ? ' tg-trending' : '');
    ...
    console.log('✅ bubble-renderer FIXED v2: reactions + trending + join sticker + input lock + auto-scroll');
